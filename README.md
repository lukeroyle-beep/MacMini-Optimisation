...
It extends the existing Buzz, OpenClaw and AI-Ops stack without modifying their databases. Phase 1 permits A0 observation and A1 preparation, plus one owner-approved scheduled A2 exception for publishing the deterministic morning brief to the private operating channel. All other A2 actions remain denied, A3 actions become approval requests routed to Codex, and A4 actions are always prohibited. The only post-commissioning production model change is the owner's explicit alignment of canonical Bumble to `gpt-5.6-terra[medium]`.
...
- Canonical Bumble is running through the Codex harness on `gpt-5.6-terra[medium]`; Pollen remains a separate legacy identity.
...
- Encrypted Time Machine backups to the external 160 GB `Mac Mini Time Machine` APFS volume. The preserved pre-existing Home Assistant archive remains on the internal disk.
...
Phase 2 and every A2 capability other than the explicitly approved daily publication remain gated on measured reliability and a separate approval decision. The 160 GB backup disk is usable but below macOS's recommended capacity for this Mac, so historical retention will be limited.
